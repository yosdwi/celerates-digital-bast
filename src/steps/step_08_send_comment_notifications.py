...
def run():
    ...
    for comment in pending_comments:
        ...
        if email_response:
            ...
            if existing and not existing.get('list'): # NocoDB API v1/v2 returns 'list'
                ...
                if creation_response:
                    ...
                    logging.error(f"Gagal mencatat notifikasi untuk comment_id {comment_id}.")
            else:
                logging.warning(f"Notifikasi untuk comment_id {comment_id} sudah dicatat. Melewatkan.")
        else:
            logging.error(f"Gagal mengirim email untuk comment_id {comment_id}.")
