...
class ClsTimesheet:
    ...
    def generate_monthly_timesheet_data(self,
        ...
        holiday_lookup = {r.get('fields', {}).get('Date'): r for r in holiday_records} if holiday_records else {}
        ...
        for employee_name, employee_info in employee_mapping.items():
            ...
            for day in month_days:
                ...
                if schedule_records is not None:
                    ...
                    schedule_fields = schedule_record.get('fields', {})
                    work_type_raw = schedule_fields.get('Work Type')
                    ...
                    if date_str in holiday_lookup:
                        holiday_val = 'H'
                        holiday_record_fields = holiday_lookup.get(date_str, {}).get('fields', {})
                        remarks_val = holiday_record_fields.get('Day Type', 'Public Holiday')
                    elif is_weekend:
                        holiday_val = 'H'
                        remarks_val = 'Weekend'
                    else:
                ...
                record = {
                    "Date": date_str,
                    "Month": month_str,
                    "Name Table": [employee_id],
                    "Activity": activity_val,
                    "Project Name": self.default_project,
                    "Start Time Table": self._get_record_link(attendance_record),
                    "End Time Table": self._get_record_link(attendance_record),
                    task_field_name: [r['id'] for r in tasks if r and r.get('id')],
                    "Holiday": holiday_val, "Remarks": remarks_val, "TTD": ""
                }
    ...
    def _get_record_link(self, record: Dict) -> Optional[List[int]]:
    ...
    def _create_lookup(self, records: List[Dict], field_key: str, is_list: bool = False) -> Dict:
        ...
        for record in records:
            key = record.get('fields', {}).get(field_key)
